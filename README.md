...
- **Codex**：ChatGPT 訂閱額度（官方 usage API，/tmp 快取 5 分鐘）；自動偵測 `~/.codex` 與 `~/.codex-*` 多帳號；顯示實際使用的 model 與 reasoning effort（有 pin 才顯示 effort）；額度視窗標籤由 API 回傳的視窗秒數自動推導
...
```bash
curl -o ~/.claude/statusline.sh https://raw.githubusercontent.com/ryanjao/claude-code-statusline/main/statusline.sh
...
預設第二行顯示當前時間。想改成「最後送出訊息的時間」，在 `~/.claude/settings.json` 加一個 UserPromptSubmit hook：
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "date +%H:%M > /tmp/claude-last-prompt-${CLAUDE_SESSION_ID}.txt"
          }
        ]
      }
    ]
